...
<#
 ...
 .\Search-Registry.ps1 -StartKey "HKLM:\SOFTWARE" -Pattern '^(?=.*?\bflash\b)(?=.*?\bAdobe\b).*$'  -MatchData -ComputerName PEN-2368-L1

 $computers = get-content "c:\temp\pcsOK.txt"
 foreach ($comp in $computers) {
   .\Search-Registry.ps1 -StartKey "HKLM:\SOFTWARE" -Pattern 'Macromed' -MatchData -ComputerName $comp
 }
