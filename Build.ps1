...
& $ilmergeExe /wildcards /lib:"$libFolder" /keyfile:"$buildFolder\Key.snk" /targetplatform:"v4,C:\Program Files (x86)\Reference Assemblies\Microsoft\Framework\.NETFramework\v4.7.1" /out:"$mergeFolder\$targetFile" "$libFolder\$targetFile"  "$mergeFolder\*.dll"
Write-Host `n
